...
export default function CarConfigurator(): React.ReactElement {
  ...
  const driveWheelChangeHandler = (driveWheel: Option) => {
    setCarConfig((currCarConfig) => {
      const nextCarConfig = produce(currCarConfig, (draft) => {
        draft.selectedDriveWheel = driveWheel;
      });
      return nextCarConfig;
    });
  };
  ...
  return (
    <div className={styles.carConfigurator}>
      ...
      <div className={styles.chosenConfig}>
        ...
        <h3>Drive Wheel: {carConfig.selectedDriveWheel?.value}</h3>
      </div>
      <div>
        {/* Pure Comp. / Presentation Comp. / Dumb Com. */}
        <OptionPicker
        ...
        <OptionPicker
          ...
          onOptionSelect={driveWheelChangeHandler}
        />
